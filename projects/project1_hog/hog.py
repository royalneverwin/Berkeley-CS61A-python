    ...
    def say(score0, score1):
        if who == 0:
            if score0 - last_score > running_high:
                print('%d point(s)! That\'s a record gain for Player %d!' % (score0 - last_score, who))
            return announce_highest(who, score0, max(running_high, score0 - last_score))
        else:
            if score1 - last_score > running_high:
                print('%d point(s)! That\'s a record gain for Player %d!' % (score1 - last_score, who))
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    ret = 0
    for i in range(1, 11):
        cur = make_averaged(roll_dice, trials_count)(i, dice)
        if average < cur:
            average = cur
            ret = i
    return ret
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if picky_piggy(opponent_score) >= cutoff:
        return 0
    elif score + picky_piggy(opponent_score) == opponent_score and 2*opponent_score-score >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if hog_pile_strategy(score, opponent_score, GOAL_SCORE - score) == 0: # 0次即可赢
        return 0
    elif GOAL_SCORE - score <= 3:
        return 1
    elif GOAL_SCORE - score <= 5:
        return 2
    elif abs(score - opponent_score) <= 6: # 差别不大就求稳
        tmp = (score-1) % 6
        if tmp == 0 or tmp == 1 or tmp == 2:
            return hog_pile_strategy(score, opponent_score, 6, 3) 
        else:
            return 0
    elif score > opponent_score:
        return hog_pile_strategy(score, opponent_score, 6, 3)
    elif abs(score - opponent_score) <= 9:
        return hog_pile_strategy(score, opponent_score, 7, 4)
    elif abs(score - opponent_score) <= 12:
        return hog_pile_strategy(score, opponent_score, 8, 5)
    else:
        return hog_pile_strategy(score, opponent_score, 8, 6)
    # END PROBLEM 12
