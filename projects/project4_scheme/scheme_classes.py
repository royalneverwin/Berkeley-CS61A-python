...
class MacroProcedure(LambdaProcedure):
    ...
    def create_env(self, values, env):
        if len(self.formals) != len(values):
            raise SchemeError('Incorrect number of arguments to function call')
        # BEGIN PROBLEM 8
        new_env = Frame(env.parent)
        new_env.bindings = env.bindings
        formals = self.formals
        while formals != nil:
            new_env.define(formals.first, values.first)
            formals = formals.rest
            values = values.rest
        return new_env
