...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dis = 0
        while dis < self.min_range or not len(cur_place.bees):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        prev_bees = self.place.bees[:] # 复制一个
        for bee in prev_bees:
    ...
    def action(self, gamestate):
        if self.chew_countdown > 0:
            ...
            if len(self.place.bees) > 0:
                cur_bee = random_bee(self.place.bees)
                cur_bee.reduce_health(cur_bee.health)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.queen_cnt == 1:
            return None
        else:
            gamestate.queen_cnt = 1
    ...
    def action(self, gamestate):
        ...
        while True:
            if cur_place.ant:
                if not cur_place.ant.is_doubled:
                    cur_place.ant.is_doubled = True
                    cur_place.ant.damage *= 2
                if cur_place.ant.is_container:
                    if cur_place.ant.ant_contained:
                        if not cur_place.ant.ant_contained.is_doubled:
                            cur_place.ant.ant_contained.is_doubled = True
                            cur_place.ant.ant_contained.damage *= 2
            if not cur_place.exit:
                break
            cur_place = cur_place.exit
        # END Problem 12
    ...
    def remove_from(self, place): # cannot be removed
        return
