...
def accuracy(typed, reference):
    ...
    if len(typed_words) > len(reference_words):
        typed_words = typed_words[0:len(reference_words)]
    elif len(typed_words) > len(reference_words):
        reference_words = reference_words[0:len(typed_words)]
    for tword, rword in zip(typed_words, reference_words):
        if tword == rword:
            right += 1
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        ret = min(valid_words, key=lambda s: diff_function(typed_word, s, limit))
        if diff_function(typed_word, ret, limit) > limit:
            return typed_word
        return ret
    # END PROBLEM 5
...
def fastest_words(match):
    ...
    for idx in word_indices:
        ...
        for id in player_indices:
            if minId == -1 or time(match, id, idx) < minT:
                minT, minId = time(match, id, idx), id
        ret[minId].append(word_at(match, idx))
    return ret
    # END PROBLEM 10
