...
def non_decrease_subseqs(s):
    ...
    """
    def subseq_helper(s, prev): # not include prev! But prev must be before s
        if not s:
            return [[]]
        elif s[0] < prev:
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    """
    # 实际上就是卡特兰数
    # 把树分成两部分 一部分i个leaves 另一部分n-i个leaves
    # 遍历i即可
    if n == 1 or n == 2:
        return 1
    else:
...
def trade(first, second):
    ...
    while sum(first[0:m]) != sum(second[0:n]) and m <= len(first) and n <= len(second):
...
def deep_len(lnk):
    ...
    if lnk == Link.empty:
        return 0
    elif not isinstance(lnk.first, Link) and lnk.rest == Link.empty:
        return 1
    else:
