...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        True
        """
        assert len(opponent.deck.cards) >= 3, 'deck has no enough cards'
        assert len(opponent.hand) >= 3, 'hand has no enough cards'
        opponent.hand.pop(0)
        opponent.hand.pop(0)
        opponent.hand.pop(0)
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        300
        """
        tmp = opponent_card.defense
        opponent_card.defense = opponent_card.attack
        opponent_card.attack = tmp 
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        while idx < len(opponent.deck.cards):
            if opponent.deck.cards[idx].attack == opponent_card.attack or opponent.deck.cards[idx].defense == opponent_card.defense:
                opponent.deck.cards.pop(idx)
            else:
                idx += 1
