...
# 重点是flag应该怎么表示 每次是+还是-
# 我的想法是用pingpong(n-1)和pingpong(n-2)的差表示 但这样太复杂 会超时
# 可以直接对flag进行一个递归 定义一个子函数flag 利用flag(n-1)得到flag(n)的值
# 直接求flag会比用pingpong(n-1)和pingpong(n-2)快很多
# 妙啊！
def pingpong(n):
...
def count_coins(change, max=25):
    ...
    if change == 0:
        ...
        return count_coins(change - max, max) + count_coins(change, descending_coin(max))
...
def move_stack(n, start, end):
    ...
    if n > 1:
        ...
        elif 2 != start and 2 != end:
            move_stack(n-1, start, 2)
        elif 3 != start and 3 != end:
            move_stack(n-1, start, 3)
    print_move(start, end)
    if n > 1:
        if 1 != start and 1 != end:
            move_stack(n-1, 1, end)
        elif 2 != start and 2 != end:
            move_stack(n-1, 2, end)
        elif 3 != start and 3 != end:
            move_stack(n-1, 3, end)
...
# 定义一个lambda 参数有一个是有两个参数的函数 返回一个函数： 这个函数接受一个n 调用f(f, n)；在定义一个lambda 参数有两个 作为第一个lambda的参数
