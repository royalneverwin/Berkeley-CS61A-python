...
class Player:
    ...
    def debate(self, other):
        prob = random()
        if prob < max(0.1, self.popularity / (self.popularity + other.popularity)):
            self.popularity += 50
        else:
...
def add_trees(t1, t2):
    ...
    if t1 == None:
        ...
        idx1, idx2 = 0, 0
        branches = []
        while idx1 < len(t1.branches) and idx2 < len(t2.branches):
            branches.append(add_trees(t1.branches[idx1], t2.branches[idx2]))
            idx1, idx2 = idx1+1, idx2+1
        
        while idx1 < len(t1.branches):
            branches.append(t1.branches[idx1])
            idx1 += 1
        while idx2 < len(t2.branches):
            branches.append(t2.branches[idx2])
            idx2 += 1
...
def match_url(text):
    ...
    scheme = r'https?://'
    domain = r'[a-zA-Z0-9.]+'
    path = r'(/[a-zA-Z0-9]+)*(.[a-zA-Z0-9]+)?' # 最多一个.
    anchor = r'/#[a-zA-Z0-9\-_]+'
